...
@Service
@AllArgsConstructor
public class ToDocxApplication {
    ...
    public void convert(Document doc) {
        ...
        try (XWPFDocument xwpfDocument = new XWPFDocument()) {
            ...
            XWPFParagraph info = xwpfDocument.createParagraph();
            info.setAlignment(ParagraphAlignment.CENTER);
            XWPFRun infoRun = info.createRun();
            infoRun.setTextPosition(20);

            // TODO get information via related
            infoRun.setText("к Договору № 0001 от 22.02.2022");
            ...
            headerRun.setText(doc.getFields().stream()
                    .filter(el -> el.getName().equals("application_header")).findFirst()
            ...
            List<DocField> docFields = doc.getFields().stream()
                    .filter(el -> el.getName().contains("application_div")).toList();
            ...
            String p11Str = doc.getFields().stream()
                    .filter(el -> el.getName().equals("application_customer_details_col")).findFirst()
                    .orElseThrow(DocFieldNotFoundException::new).getDefaultValue();
            ...
            String p12Str = doc.getFields().stream()
                    .filter(el -> el.getName().equals("application_executor_details_col")).findFirst()
                    .orElseThrow(DocFieldNotFoundException::new).getDefaultValue();
            ...
            String p21Str = doc.getFields().stream()
                    .filter(el -> el.getName().equals("application_customer_sign")).findFirst()
                    .orElseThrow(DocFieldNotFoundException::new).getDefaultValue();
            ...
            String p22Str = doc.getFields().stream()
                    .filter(el -> el.getName().equals("application_executor_sign")).findFirst()
                    .orElseThrow(DocFieldNotFoundException::new).getDefaultValue();
